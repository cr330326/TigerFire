...
| ^ | ^ | 新增 | 音效系统设计：BGM + 场景差异化音效 + 语音规范 | 3.6 |
...
| v1.1 | 2026-01-25 | 新增 | 稳定性要求：崩溃率、白屏率、ANR 率、内存泄漏指标 | 3.2 |
| ^ | ^ | 新增 | 崩溃日志与错误追踪：日志存储、格式、内容规范 | 3.3 |
| ^ | ^ | 更新 | 验收标准：新增稳定性指标和崩溃日志系统验收项 | 6.2 |
...
- 延迟100ms直接进入主地图页面，欢迎页把任意点击屏幕按钮显示去掉
...
- 小火语音提示："学校着火啦！快叫消防车！"
- 然后有个图标提示用户点击播放视频，点击后播放视频，视频资源名称叫做School_Fire_Safety_Knowledge.mp4。
- 等视频播放完成 → 弹出 **小火点赞动画（Lottie）** + 语音："你真棒！记住，着火要找大人帮忙！" → **弹出 1 枚徽章** → 解锁"森林"场景

#### 5. 森林场景（点击交互救援）

- 进入后显示森林火灾画面，小火语音提示："小羊被困啦！快开直升机救它们！"
- 屏幕左侧显示 **直升机图标（超大，≥150pt）**，然后一直在做起螺旋桨飞行的动画
- 屏幕右侧显示两只小羊图标，小羊周边被火苗包围，小羊图标做着求救的动画
- 用户点击小羊图标1，直升机图标就缓慢飞行到小羊图标1上方，并且显示放下梯子按钮（圆形，≥100pt）
- 点击按钮 →播放视频 rescue_sheep_1.mp4内容
- 用户点击小羊图标2，直升机图标就缓慢飞行到小羊图标2上方，并且显示放下梯子按钮（圆形，≥100pt）
- 点击按钮 →播放视频 rescue_sheep_2.mp4内容
- 最后等两个视频都播放完了，成功救出两只小羊 → 播放 庆祝 Lottie 动画 + 语音总结："直升机能从天上救人，真厉害！"
...
| 项目 | 要求 |
|------|------|
| **存储位置** | 应用数据目录（Android: `filesDir/crash_logs/`，iOS: `Application Support/crash_logs/`） |
| **日志格式** | JSON 文件，便于后续分析 |
...
```json
{
  ...
  "stackTrace": "完整堆栈跟踪信息",
  "scene": "firestation",          // 崩溃时所在场景
  "userAction": "点击设备按钮",     // 用户最后操作
  "memoryUsage": "85MB",           // 当前内存占用
  "deviceFreeMemory": "512MB"      // 设备可用内存
}
```
...
除崩溃外，以下情况也需记录日志：
- 视频加载失败（含资源路径、错误原因）
- Lottie 动画解析失败（含文件名、错误信息）
- 数据库读写异常（含操作类型、错误信息）
- 内存警告（Android: `onLowMemory()`，iOS: `didReceiveMemoryWarning`）
...
**稳定性指标：**
- [ ] 端到端测试（完整游玩 10 轮）崩溃次数为 0
- [ ] 长时间运行测试（连续运行 2 小时）无内存泄漏（内存占用稳定在 ±10% 范围内）
- [ ] 场景切换压力测试（快速切换 100 次）无白屏、无崩溃
- [ ] 低端设备测试（1GB RAM Android）所有场景可正常运行

**崩溃日志系统：**
- [ ] 应用崩溃后能自动生成日志文件并保存到指定目录
- [ ] 日志文件包含完整堆栈信息、设备信息、场景信息
- [ ] 日志文件数量超过 20 个时自动清理最旧的
- [ ] 日志不包含任何用户可识别信息（PII）
- [ ] 非致命错误（视频加载失败、动画解析失败等）也能正确记录
